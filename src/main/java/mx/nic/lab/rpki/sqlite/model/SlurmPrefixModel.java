...
public class SlurmPrefixModel {
	 ...
	 * @throws SQLException
	 */
	public static List<SlurmPrefix> getAll(int limit, int offset, LinkedHashMap<String, String> sort,
			Connection connection) throws SQLException {
		String query = getQueryGroup().getQuery(GET_ALL);
		query = Util.getQueryWithPaging(query, limit, offset, sort, SlurmPrefixDbObject.propertyToColumnMap);
		try (PreparedStatement statement = connection.prepareStatement(query)) {
			logger.log(Level.INFO, "Executing QUERY: " + statement.toString());
			ResultSet rs = statement.executeQuery();
			if (!rs.next()) {
	 ...
	 * @throws SQLException
	 */
	public static List<SlurmPrefix> getAllByType(int type, int limit, int offset, LinkedHashMap<String, String> sort,
			Connection connection) throws SQLException {
